...
class KbDataset(Dataset):
    ...
    def negative_sampling_kgbert(self, raw_kg_triplet, weights=None, *args, **kwargs):
        head, rel, tail = raw_kg_triplet[:3]
        rnd = random.random()
        while True:
            if rnd < 0.5:  # replace
                pos_ent_set = set(self.obj2subjs[tail][rel])
                sampled = head
                while sampled == head or sampled in pos_ent_set:
                    sampled = random.choice(self.ent_list)
                neg_example = [sampled, rel, tail]
            else:
                pos_ent_set = set(self.subj2objs[head][rel])
                sampled = tail
                while sampled == tail or sampled in pos_ent_set:
                    sampled = random.choice(self.ent_list)
                neg_example = [head, rel, sampled]
            if self.pos_triplet_str_set is not None and self._triplet2str(neg_example) in self.pos_triplet_str_set:
                continue
            else:
                break
